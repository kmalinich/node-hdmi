...
// Output log message to log.msg
function fmt_log(message) {
	log.msg({
		src : module_name,
		msg : message,
	});
...
module.exports = {
	...
	startup : (startup_callback) => {
		...
		socket.io.on('connect', () => {
			fmt_log('Connected to ' + config.server.host + ':' + config.server.port);
		...
		socket.io.on('connect_error', (error) => {
			...
			fmt_log('Connect error: ' + error);
		});

		socket.io.on('connect_timeout', () => {
			status.server.connected = false;
			fmt_log('Connect timeout');
		});
		...
		socket.io.on('reconnect', (number) => {
			...
			fmt_log('Reconnected after ' + number + ' tries');
		});
		...
		socket.io.on('disconnect', () => {
			status.server.connected = false;
			fmt_log('Disconnected from server');
		});
